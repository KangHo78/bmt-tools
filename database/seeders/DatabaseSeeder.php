...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $running = Loan::create(['trx_no' => 'TRX-1042', 'user_id' => $budi->id, 'usage_type' => 'dalam_area', 'purpose' => 'Pemasangan rak gudang B', 'location_text' => 'Gudang B - Lantai 2', 'start_date' => now()->subDays(2), 'due_date' => now()->addDay(), 'status' => 'berjalan', 'tokens_used' => 2, 'handover_at' => now()->subDays(2)]);
        foreach ([['TWL-DRL', 2], ['TWL-HLM', 2]] as [$code, $idx]) {
            $unit = $units[$code][$idx];
            $unit->update(['status' => 'dipinjam']);
            $running->items()->create(['tool_type_id' => $types[$code]->id, 'unit_id' => $unit->id, 'condition_out' => 'baik', 'checklist' => array_fill_keys($types[$code]->checklist, true)]);
        }

        $pending = Loan::create(['trx_no' => 'TRX-1058', 'user_id' => $fajar->id, 'usage_type' => 'luar_area', 'purpose' => 'Renovasi kantor cabang Bekasi', 'location_text' => 'Kantor Cabang Bekasi', 'start_date' => now()->addDay(), 'due_date' => now()->addDays(10), 'status' => 'menunggu_approval', 'letter_url' => '/demo/surat-peminjaman.pdf', 'tokens_used' => 2]);
        foreach (['TWL-TGA', 'TWL-HAR'] as $code) {
            $pending->items()->create(['tool_type_id' => $types[$code]->id]);
        }

        $late = Loan::create(['trx_no' => 'TRX-0998', 'user_id' => $budi->id, 'usage_type' => 'dalam_area', 'purpose' => 'Pemeliharaan tangga darurat', 'location_text' => 'Gedung A', 'start_date' => now()->subDays(10), 'due_date' => now()->subDays(6), 'status' => 'terlambat', 'tokens_used' => 1, 'handover_at' => now()->subDays(10)]);
        $units['TWL-TGA'][2]->update(['status' => 'dipinjam']);
        $late->items()->create(['tool_type_id' => $types['TWL-TGA']->id, 'unit_id' => $units['TWL-TGA'][2]->id, 'condition_out' => 'baik']);
